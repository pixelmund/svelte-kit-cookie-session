...
export function encrypt(encryptionKey: BinaryLike): (text: string) => string {
  ...
  return (text) => {
    ...
    const key = pbkdf2Sync(
      ...
      keyLengthHint(encryptionOptions.algorithm),
    ...
    return Buffer.concat([salt, iv, cipher.getAuthTag(), cipherText]).toString(
      encryptionOptions.outputEncoding
    );
  };
}
...
export function decrypt(
  ...
  return (encrypted_string) => {
    const buffer = Buffer.from(
      encrypted_string,
      encryptionOptions.inputEncoding
    );
    // data is packed in this sequence [salt iv tag cipherTest]
    const tagStartIndex =
      encryptionOptions.saltLength + encryptionOptions.ivLength;
    ...
    return `${decipher.update(
      cipherText,
      "binary",
      encryptionOptions.stringEncoding
    )}${decipher.final(encryptionOptions.stringEncoding)}`;
  };
}
